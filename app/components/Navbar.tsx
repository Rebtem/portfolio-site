...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full bg-black bg-opacity-50 dark:bg-gray-900 text-white z-50 px-4 py-3 flex justify-between items-center md:h-full md:w-16 md:flex-col">
      ...
      <div
        id="mobile-menu"
        className={`fixed top-0 left-0 h-full w-64 bg-black bg-opacity-90 transform ${
          menuOpen ? "translate-x-0" : "-translate-x-full"
        } transition-transform duration-300 md:hidden flex flex-col items-center space-y-4 pt-16`}
      >
        {menuItems.map((item, index) => (
          <Link
            key={index}
            href={item.href}
            className="w-full text-center py-3 hover:bg-gray-700"
          >
            {item.name}
          </Link>
        ))}
      ...
      <div className="hidden md:flex md:flex-col md:items-center md:space-y-4">
        {menuItems.map((item, index) => (
          <Link
            key={index}
            href={item.href}
            className="relative group my-2 md:my-4"
          >
